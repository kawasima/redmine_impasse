...
jQuery(document).ready(function($) {
	/*
	 * função que devera sobrescrever a função show_test_case quando confocada
	 */
	...
	$("#executions-view form").live("submit", function(e) {
		...
		if (execution_status == "2") {// NG
			post_save_function = function() {
				$.get(IMPASSE.url.executionBugsNew, {}, function(data) {
					$.unblockUI();
	...
	$("#issue-dialog #button-create-issue").die('click').live("click", function(e) {
		$.ajax({
			...
			type : 'POST',
			data : $("#issue-form").serialize() + "&execution_bug[execution_id]=" + $("#executions-view :hidden#execution_id").val(),
			success : function(data) {
				if (data.errors) {
